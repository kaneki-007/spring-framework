...
public abstract class AbstractRefreshableWebApplicationContext extends AbstractRefreshableConfigApplicationContext
	...
	protected void postProcessBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		/**
		 * 1、添加 ServletContextAwareProcessor 到 BeanFactory 容器中，该 processor 实现 BeanPostProcessor 接口，
		...
		/**
		 * 3、注册 WEB 应用特定的域（scope）到 beanFactory 中，以便 WebApplicationContext 可以使用它们。
		 * 比如 "request" , "session" , "globalSession" , "application"
		 */
		WebApplicationContextUtils.registerWebApplicationScopes(beanFactory, this.servletContext);

		/**
		 * 4、注册 WEB 应用特定的 Environment bean 到 beanFactory 中，以便WebApplicationContext 可以使用它们。
